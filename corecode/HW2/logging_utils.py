...
def setup_experiment_logger(config: Config) -> Tuple[logging.Logger, str]:
    ...
    # Clear stale handlers to avoid duplicate output across re-runs
    if logger.handlers:
...
def save_config_snapshot(config: Config, log_path: str) -> str:
    ...
    Args:
        config: Full experiment configuration.
        log_path: Path to the log file (snapshot is written alongside it).
...
def log_environment_info(logger: logging.Logger) -> None:
    """Log Python, PyTorch, and hardware environment information.
