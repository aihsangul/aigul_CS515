...
# ── CIFAR-10 class names (used for logging / plotting) ────────────────────────
CIFAR10_CLASSES = (
...
def get_train_transforms(config: Config) -> transforms.Compose:
    """Build training-time data transforms for the given training mode.

    For ``transfer_resize`` the images are upscaled to 224×224 and normalised
    with ImageNet statistics (matching the pre-training distribution). For all
    other modes, standard CIFAR-10 augmentation (random crop + horizontal flip)
    is applied with CIFAR-10 channel statistics.
    ...
    """
    if config.train.training_mode == "transfer_resize":
        return transforms.Compose([
            transforms.Resize(256),
            transforms.RandomCrop(224),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
            transforms.Normalize(config.data.imagenet_mean, config.data.imagenet_std),
        ])
...
def get_val_transforms(config: Config) -> transforms.Compose:
    """Build validation / test transforms for the given training mode.
...
def get_train_val_loaders(config: Config) -> Tuple[DataLoader, DataLoader]:
    """Create train and validation DataLoaders by splitting the CIFAR-10 train set.

    The official test set is held out for final evaluation only. A fraction
    ``val_split`` of the training set is used for validation.
...
def build_optimizer(
    ...
    Args:
        params: Iterable of parameters (or parameter groups) to optimise.
        config: Full experiment configuration.
    ...
    Raises:
        ValueError: If the requested optimiser is not supported.
    """
...
def build_scheduler(
    ...
    Supported schedulers: ``none``, ``step``, ``plateau``, ``cosine``.
    ...
    Raises:
        ValueError: If the requested scheduler is not supported.
    """
...
def compute_kd_loss(
    ...
    The T^2 factor re-scales the soft targets to match the magnitude of the
    hard-label gradients, as recommended by Hinton et al. (2015).
...
def compute_kd_ls_loss(
    ...
    Instead of using all teacher soft-label probabilities, the teacher is only
    used to assign a probability to the **true class**. The remaining probability
    mass is spread equally across all other classes:

        soft_label[true_class]   = p_teacher[true_class]
        soft_label[other_classes] = (1 - p_teacher[true_class]) / (C - 1)

    This injects example-level difficulty from the teacher while keeping the
    soft distribution simple (cf. Yuan et al., CVPR 2020).

    Args:
        student_logits: Raw logits from the student, shape (B, C).
        teacher_logits: Raw logits from the teacher (detached), shape (B, C).
        labels: Ground-truth class indices, shape (B,).
        temperature: Temperature used when converting teacher logits to probs.
    ...
    true_class_prob = teacher_probs[torch.arange(batch_size, device=device), labels]  # (B,)
    ...
    # Assign remaining probability equally to non-true classes
    remaining = (1.0 - true_class_prob) / (num_classes - 1)          # (B,)
    soft_labels += remaining.unsqueeze(1)                             # broadcast
...
def train_one_epoch(
    ...
    For knowledge distillation modes (``kd`` / ``kd_ls``) a frozen teacher
    model must be provided. The teacher is evaluated in ``torch.no_grad()``
    context so its parameters are never updated.

    For standard modes the ``criterion`` argument is used as the loss function.

    Args:
        model: Student (or solo) model to train.
        loader: Training DataLoader.
        optimizer: Optimiser instance.
        criterion: Loss function for non-KD modes (ignored for kd/kd_ls).
        device: Target device.
        config: Full experiment configuration.
        teacher: Frozen teacher model (required for kd / kd_ls modes).
    ...
    for batch_idx, (images, labels) in enumerate(loader):
        ...
        if mode == "kd":
            ...
            loss = compute_kd_loss(
                ...
                teacher_logits,
                labels,
                config.train.kd_temperature,
                config.train.kd_alpha,
            )
        elif mode == "kd_ls":
            with torch.no_grad():
                teacher_logits = teacher(images)  # type: ignore[misc]
...
def validate(
    ...
    Uses standard CE loss regardless of training mode so validation loss is
    always interpretable as a classification metric.

    Args:
        model: Model to evaluate.
        loader: Validation DataLoader.
        criterion: Standard CE loss (no label smoothing).
        device: Target device.
...
def run_training(
    ...
    Supports all six training modes defined in ``parameters.TRAINING_MODES``.
    The teacher model (if provided) is expected to be already loaded, moved to
    ``device``, and frozen (``requires_grad=False`` on all parameters).

    Key behaviours:
    - Training / validation split from the official CIFAR-10 training set.
    - Best-model checkpoint saved on validation-loss improvement.
    - Early stopping based on validation loss with configurable patience.
    - Training history JSON saved alongside the checkpoint.

    Args:
        model: Student (or solo) model to train.
        config: Full experiment configuration.
        device: Target device.
        teacher: Frozen teacher model for KD modes; ``None`` for other modes.
    ...
    # Training loss: label smoothing for scratch_ls; plain CE for everything else
    # (KD modes use their own loss functions inside train_one_epoch)
    if config.train.training_mode == "scratch_ls":
    ...
    # Validation always uses plain CE for a fair comparison across modes
    val_criterion = nn.CrossEntropyLoss()

    # Only optimise parameters that require gradients (handles frozen backbone)
    ...
    for epoch in range(1, config.train.epochs + 1):
        ...
        if improved:
            ...
            logger.info(
                "  Saved best model → %s (val_loss=%.4f)",
            ...
            patience_counter += 1
            logger.info(
                "  No improvement. Patience counter: %d/%d",
                patience_counter,
